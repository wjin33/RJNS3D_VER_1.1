function [beta_loc,np1,np2,is_jmw_flag]=IsJointMWInsect(joint_c,j_dd_dip,h_j,p_cn1,p_cn2,mw_dd_dip)
...
j_nv=nto*[0.0;0.0;10]; %normal vector of the measuring window in global coordinate system
o_lc=[j_nv(1,1);j_nv(2,1);j_nv(3,1);-(j_nv')*cs_o;];%generate coefficients vector of the measuring window equation (oax+oby+ocz+od=0) in global coordinate system
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
n_lc=[((j_nv')*nto(:,1));((j_nv')*nto(:,2));((j_nv')*nto(:,3));((j_nv')*cs_o+o_lc(4,1));];
...
beta_loc=[];%store the local angle
...
if np1(1,1)==np2(1,1)
    beta_loc=0;
elseif np1(2,1)==np2(2,1)
    beta_loc=pi/2;
else
    beta_loc=pi-atan(-(np1(1,1)-np2(1,1))/(np1(2,1)-np2(2,1)));
end
opl=zeros(row,col);
for i=1:col
...
if is_flag==0
	...
	is_jmw_flag=1;
	np1=PruneMartix(np1,1e-5);
    np2=PruneMartix(np2,1e-5);
