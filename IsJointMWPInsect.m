function [np1,np2,is_jmw_flag]=IsJointMWPInsect(joint_c,j_dd_dip,h_j,p_cn1,p_cn2,mw_dd_dip)
...
%np_cn1--3x1 matrix, the local coordinates of left lower point of rectangular which is made of cuting through silulation region by measuring window
%np_cn2--3x1 matrix, the local coordinates of right upper point of rectangular which is made of cuting through silulation region by measuring window
...
%region_size--6x1 matrix,the simulation region (x0;y0;z0;dx;dy;dz)
...
beta=mw_dd_dip(2,1);%the rotato angle of dip 
...
n_lc=[((wm_nv')*nto(:,1));((wm_nv')*nto(:,2));((wm_nv')*nto(:,3));((wm_nv')*cs_o+o_lc(4,1));];
